...
@RestController
@RequestMapping("/api/collections-kafka")
public class CollectionsKafkaResource {

    private final Logger log = LoggerFactory.getLogger(CollectionsKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public CollectionsKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
