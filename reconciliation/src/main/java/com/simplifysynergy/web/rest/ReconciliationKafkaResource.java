...
@RestController
@RequestMapping("/api/reconciliation-kafka")
public class ReconciliationKafkaResource {

    private final Logger log = LoggerFactory.getLogger(ReconciliationKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public ReconciliationKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
